...
struct Node {
    ...
    Node( vector<int> position_v, Graph* graph_v, float cost_v = 0){
        position = position_v;
        neighbours = {};
        parent = nullptr;
        cost = cost_v;
                ...
                if (newX >= 0 && newX <= graph->map_dim-1 && newY >= 0 && newY <= graph->map_dim-1) {
                    ...
                    if (nodeAlreadyExists == false) {
                        ...
                        if ((i == 1 && j == 1) || graph->map[newX][newY] == zero_vec ) {
                            ...
                            Node* neigh = new Node( {newX , newY}, graph, 1000);
...
pair< vector<Node*> , vector<Node*> > Dijkstra(Graph* graph){
    ...
    while(open_list.size()){ // while open list is not empty
        ...
        if (current_node->position == graph->goal_position){
            ...
            for (auto i : current_node->neighbours){
                float neighbour_cost = current_node->cost + dist(current_node->position,i->position);
...
int main() {
    ...
    pair< vector<Node*> , vector<Node*> > sol = Dijkstra(&My_Graph); // Solve for the path
